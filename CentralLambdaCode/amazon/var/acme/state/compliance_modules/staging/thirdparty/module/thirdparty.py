...
class ThirdPartyMacOSComplianceModule(compliance.BaseModule):
    """
    Compliance Module for macOS which will evaluate third party patch compliance
    """
    
    logger_name = "ThirdPartyMacOS"
    auto_remediate = False
    can_remediate = False
    thirdparty_tool_path = "/usr/local/amazon/bin/acmeupdates"
    patches_pref_path = "/Library/Preferences/com.amazon.acme.updates.plist"
    thirdparty_domain = "com.amazon"
    
    def __init__(self,key_map=None,settings_keys=None,*args,**kwargs):
                
        self.identifier = "thirdparty"
        self.name = "ThirdPartyMacOS"
        ...
        if key_map is None:
            ...
            key_map["thirdparty_tool_path"] = None
            key_map["patches_pref_path"] = None
            key_map["thirdparty_domain"] = None
        
        if settings_keys is None:
            settings_keys = compliance.BaseModule.settings_keys[:]
            settings_keys.append("thirdparty_tool_path")
            settings_keys.append("patches_pref_path")
            settings_keys.append("thirdparty_domain")
        
        super(ThirdPartyMacOSComplianceModule, self).__init__(name=self.name,
    ...
    def evaluate_(self, *args, **kwargs):
        """
        Method to evaluate third party patch compliance    
        """
        ...
        if not os.path.exists(self.thirdparty_tool_path):
            logger.warning("Third Party Patches tool not found at {}.".format(self.thirdparty_tool_path))
            status = compliance.ComplianceStatus.ERROR
        else:
            cmd = [self.thirdparty_tool_path, "--listUpdates"]
            try:
                output = subprocess.check_output(cmd)
                if not os.path.exists(self.patches_pref_path):
                    logger.error("Third Party Patches preferences file was not found at {}.".format(self.patches_pref_path))
                    status = compliance.ComplianceStatus.ERROR
                else:
                    ...
                    try:
                        ...
                        logger.error("Missing lastSWUpdatecheck key in Third Party Patches preferences file at {}. Setting stale state.".format(self.patches_pref_path))
                        status = compliance.ComplianceStatus.ERROR
                    if abs(datetime.datetime.utcnow() - last_check) >= datetime.timedelta(days=1):
                        logger.warning("LastCheck key indicates Third Party Patches preferences file is stale.")
                        status = compliance.ComplianceStatus.ERROR
                    else:
                        for patch in patch_list:                
                            if self.patch_eligible(patch_list[patch], self.thirdparty_domain):
                ...
                logger.error("Failed to run Third Party Patches tool:{}".format(exp))
